...
Future 是 Java5 新加的一个接口，定义了操作**异步**任务执行的一些方法
...
![600](assets/Pasted%20image%2020240407110102.png)
...
继承关系：FutureTask 接口继承了 Runnable 接口 和 Future 接口

![400](assets/Pasted%20image%2020231120205347.png)

在源码中可以看到：

- FutureTask 实现了 `RunnableFuture` 接口
- FutureTask 不支持空参构造，仅支持构造传入 Runnable 和 Callable（有返回值、可抛出异常）
...
> 【案例】：FutureTask 配合 Thread 执行异步任务，获取结果
...
- 方案一，3 个任务 1 个 main 线程处理，大概 1101ms
- 方案二，3 个任务开启多个异步任务线程处理，利用线程池（假如每次 new 一个 Thread，太浪费资源，会有 GC 这些工作），大概 848ms
...
public class FutureThreadPoolDemo {
    ...
    private static void m1() throws InterruptedException {
        ...
        System.out.println("串行执行---cost time: " + (endTime - startTime));
    }
    ...
    private static void m2() throws InterruptedException {
        ...
        System.out.println("Future并行执行---cost time: " + (endTime - startTime));
    }
}
```
...
- `get()` 阻塞
	- 一旦调用 `get()` 方法，不管是否计算完成，都会导致阻塞（所以一般 get 方法放到最后）
...
对于简单的业务场景使用 Future 完全 OK，为什么会出现 CompletableFuture？

- `get()` 阻寒的方式和异步编程的设计理念相违背
- `isDone()` 轮询的方式会耗费无谓的 CPU 资源
- 对于真正的异步处理我们希望是可以通过传入回调函数，在 Future 结束时自动调用该回调函数，这样，我们就不用等待结果

因此，JDK8 设计出 `CompletableFuture`。`CompletableFuture` 提供了一种观察者模式类似的机制，可以让任务执行完成后通知监听的一方
...
![500](assets/Pasted%20image%2020231122133337.png)
