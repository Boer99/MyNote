...
1. Althogh the authors state that 'Multi-class ocean front detection aims to identify and distinguish various classes of ocean fronts at different regions' in the abstract, ==the concept of ocean front class is not clear==. If the authors focus on the mall target fronts, the need or siginifcance of the study should be explained. =='Multi-class' means different (ocean fronts)?== 
	- 1.1 是的，海洋锋类别是在海洋学上根据海洋锋形成的海域和形成机理对海洋锋分类的结果，Multi-class 是指 不同类别的海洋锋，我们补充了海洋锋类别的概念在【1.1】标注处。
	- 1.2 是的，小目标锋并不是海洋学上的概念，而是海洋锋在图像上呈现出小目标的特点，他们在视觉呈现上也非常重要
2. Quite a few low-level mistakes have been made in this manuscript, e.g., SCSOF terminology has been used several times prior to description, lack of legend in Fig.1, the description of the figure titles in Figures 2 and 4 is not standardised, incorrect citation of figure numbers, incomplete or incorrect interpretation of symbols in formulas...  
	...
	- 1.6
	- 1.7
1. The SST data is one channel, not three. If it is treated as three channels, there is a problem.
	- 1.8 是的，SST 是单通道的，SST 转换为 SST 梯度后根据 jet 颜色映射规则转换为rgb图像，再作为模型的输入，所以输入图像是三通道的，我们在【】标注处修改了论述

> 意见一：
> - 虽然作者在摘要中指出“多类海洋锋检测旨在识别和区分不同区域的各种海洋锋类别”，但海洋锋类别的概念并不明确。
> - 如果作者==关注小目标锋==，则应解释研究的必要性或意义。
> - “多类”是指不同（海洋锋）吗？ 
> 
> 意见二：
> - 本文有不少低级错误，例如，
...
- 2.2 我们补充了引用[7]和[8]，并在【2.2】标注处添加了对这两篇文献的介绍
...
- 2.3 我们仔细的阅读了 [https://doi.org/10.48550/arXiv.1708.02002](https://doi.org/10.48550/arXiv.1708.02002) ，得出 focal Loss 是在 cross entropy (CE) loss 的基础上设置了一个权重因子，根据样本分类的表现动态地控制整个样本的损失。
...
- 2.6 经典的任务是指遥感影像分割和生物医学图像分割，我们在【2.6】标注的地方补充了具体的经典分割挑战和数据集
...
- 2.7 是的，仅在我们构建的SOSOF数据集上评估我们的模型忽略了数据集偏差的我们，在结论中我们额外讨论了后续工作，包括将我们的模型推广到其他遥感数据集 和 对其进一步改进使其应用到其他领域。
...
- 2.9 是的，ocean fronts 应该为复数形式，该段落改动较大，我们对其进行了重写，见【2.9】标注处
...
- 2.11 是的，锋的名字很长，所以我们用 front1 -front10 代替了具体的锋的名字，我们在图 3 中给出了 front1-front10 具体对应的名字
