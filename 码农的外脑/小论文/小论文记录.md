...
每个块包含一个侧分支结构，其中嵌入了一个深度残差扩张卷积（DRDC），用于捕获多尺度特征，以及一个注意模块（AM），用于从通道维度和<span style="background:#d4b106">空间维度</span>中去除背景噪声，从而实现跨通道交互。
...
2）The designed bi-directional network can progressively fuse <span style="background:#d4b106">multiscale feature representations.</span> Moreover, we propose the DRDC that can fully exploit the multiscale ocean front features and the AM that can refine the feature map.
