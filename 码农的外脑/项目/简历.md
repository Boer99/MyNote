...
- 实现了 Java 原生代码沙箱。使用 Java Runtime 对用户代码编译和执行，并通过 **Process 类** 的输入流获取执行结果；选用模板方法模式定义了一套标准的代码沙箱执行流程，并允许子类自行扩展部分流程。
- 保证代码沙箱的安全性：模拟多种程序异常情况并针对性解决，如超时中断、内存限制、代码校验等；代码沙箱服务调用采用 API 签名认证的方式，防止用户恶意请求代码沙箱服务。
